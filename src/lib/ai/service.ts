...
import { createApiAiProvider } from "./api-provider";
...
/** Offline fallback when API is unreachable (e.g. static export). */
class MockAiProvider implements AiAnalysisProvider {
  async analyzeResume(file: File): Promise<CareerAnalysis> {
    ...
    return { ...mockAnalysis, candidateName: "Not Found" };
  }
}

let provider: AiAnalysisProvider = createApiAiProvider();
